...
def reserva_detalle(id):
    ...
    try:
        ...
        # Normalizo horas a string HH:MM
...
def reservas_para_resenar():
    ...
    cur = con.cursor(dictionary=True)
    try:
        # Reservas finalizadas, donde el usuario asistió y aún no reseñó
        cur.execute("""
            SELECT
                r.id_reserva,
                r.nombre_sala,
        ...
        filas = cur.fetchall()
        # Normalizar horas a string HH:MM
        for f in filas:
